            ...
            {range(numOfGroups).map((groupIndex) => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup).map((index) => {
                  ...
                  const layoutId = `item-${globalItemIndex}-${id}`;
                  return (
                    <motion.div
                      key={layoutId}
                      layoutId={layoutId}
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(remainder).map((index) => {
              const globalItemIndex =
                numOfGroups * numOfItemsPerGroup + remainder - index - 1; //So we can count backwards to add the circles at the end and not at the start
              const layoutId = `item-${globalItemIndex}-${id}`;
              return (
                <motion.div
                  key={layoutId}
